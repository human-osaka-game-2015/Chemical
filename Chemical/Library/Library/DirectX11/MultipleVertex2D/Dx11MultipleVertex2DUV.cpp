...
// Include
//----------------------------------------------------------------------
#include "Dx11MultipleVertex2DUV.h"
...
namespace Lib
{
	namespace Dx11
	{
		...
		//----------------------------------------------------------------------
		MultipleVertex2DUV::MultipleVertex2DUV() :
		...
		// Public Functions
		//----------------------------------------------------------------------
		bool MultipleVertex2DUV::Initialize(GraphicsDevice* _pGraphicsDevice)
		{
		...
		void MultipleVertex2DUV::Finalize()
		{
			ReleaseState();
			ReleasePixelShader();
			ReleaseVertexLayout();
		...
		bool MultipleVertex2DUV::CreateVertexBuffer(
		...
		bool MultipleVertex2DUV::CreateInstanceBufferUV(D3DXMATRIX* _pMat, D3DXVECTOR2* _pUV, int _instanceNum)
		{
			m_InstanceNum = _instanceNum;

			m_pInstanceDataUV = new INSTANCE_DATA_UV[m_InstanceNum];
			for (int i = 0; i < m_InstanceNum; i++)
			{
				D3DXMatrixTranspose(&m_pInstanceDataUV[i].Mat, &_pMat[i]);
				m_pInstanceDataUV[i].UV = D3DXVECTOR4(_pUV[i].x, _pUV[i].y, 0, 0);
			}

			// インスタンスバッファの設定.
			D3D11_BUFFER_DESC InstanceBufferDesc;
			ZeroMemory(&InstanceBufferDesc, sizeof(D3D11_BUFFER_DESC));
			InstanceBufferDesc.ByteWidth = sizeof(INSTANCE_DATA_UV) * m_InstanceNum;
			...
			InstanceResourceData.pSysMem = m_pInstanceDataUV;
		...
		void MultipleVertex2DUV::ReleaseVertexBuffer()
		{
			SafeRelease(m_pVertexBuffer);
		}

		void MultipleVertex2DUV::ReleaseInstanceBufferUV()
		{
			SafeDeleteArray(m_pInstanceDataUV);
			SafeRelease(m_pInstanceBuffer);
		}

		bool MultipleVertex2DUV::WriteConstantBuffer(D3DXVECTOR2 _pos)
		{
		...
		bool MultipleVertex2DUV::WriteVertexBuffer()
		{
			D3D11_MAPPED_SUBRESOURCE MappedResource;
			if (SUCCEEDED(m_pGraphicsDevice->GetDeviceContext()->Map(
				m_pVertexBuffer,
		...
		void MultipleVertex2DUV::SetVertex(const D3DXVECTOR2* _pSize)
		{
		...
		void MultipleVertex2DUV::SetUV(const D3DXVECTOR2* _pMinUV, const D3DXVECTOR2* _pMaxUV)
		{
		...
		void MultipleVertex2DUV::SetColor(const D3DXCOLOR* _pColor)
		{
		...
		bool MultipleVertex2DUV::WriteInstanceBuffer(const D3DXVECTOR2* _pDrawPos, const D3DXVECTOR2* _pUV)
		{
			D3D11_MAPPED_SUBRESOURCE MappedResource;
			if (SUCCEEDED(m_pGraphicsDevice->GetDeviceContext()->Map(
				m_pInstanceBuffer,
				...
				&MappedResource)))
			{
				INSTANCE_DATA_UV* pInstanceData = reinterpret_cast<INSTANCE_DATA_UV*>(MappedResource.pData);
				...
				for (int i = 0; i < m_InstanceNum; i++)
				{
					...
					pInstanceData[i].UV = D3DXVECTOR4(_pUV[i].x, _pUV[i].y, 0, 0);
				}
		...
		void MultipleVertex2DUV::ShaderSetup()
		{
		...
		void MultipleVertex2DUV::TextureSetup()
		{
		...
		void MultipleVertex2DUV::DepthStencilStateSetup()
		{
			m_pGraphicsDevice->GetDeviceContext()->OMSetDepthStencilState(m_pDepthStencilState, 0);
		}

		void MultipleVertex2DUV::BlendStateSetup()
		{
			m_pGraphicsDevice->GetDeviceContext()->OMSetBlendState(m_pBlendState, nullptr, 0xffffffff);
		}

		void MultipleVertex2DUV::DefaultDraw(const D3DXVECTOR2* _pDrawPos, const D3DXVECTOR2* _pUV, int _instanceNum)
		{
			if (!WriteInstanceBuffer(_pDrawPos, _pUV))	return;
		...
		void MultipleVertex2DUV::Draw(int _instanceNum)
		{
			if (_instanceNum == 0) _instanceNum = m_InstanceNum;

			ID3D11Buffer* pBuffer[2] = { m_pVertexBuffer, m_pInstanceBuffer };
			UINT Stride[2] = { sizeof(VERTEX), sizeof(INSTANCE_DATA_UV) };
			UINT Offset[2] = { 0, 0 };
			m_pGraphicsDevice->GetDeviceContext()->IASetVertexBuffers(0, 2, pBuffer, Stride, Offset);

			m_pGraphicsDevice->GetDeviceContext()->DrawInstanced(VERTEX_NUM, _instanceNum, 0, 0);
		}
		...
		// Private Functions
		//----------------------------------------------------------------------
		bool MultipleVertex2DUV::CreateVertexShader()
		{
			...
			if (FAILED(D3DX11CompileFromFile(
				TEXT("Library\\DirectX11\\Vertex2D\\MultipleVertex2D\\Effect\\Vertex2DUV.fx"),
				nullptr,
				nullptr,
				"VS",
				"vs_5_0",
		...
		bool MultipleVertex2DUV::CreateVertexLayout()
		{
			D3D11_INPUT_ELEMENT_DESC InputElementDescUV[] =
			{
				...
				{ "TEXUV",	  0, DXGI_FORMAT_R32G32_FLOAT,       1, 64, D3D11_INPUT_PER_INSTANCE_DATA, 1 }
			};

			if (FAILED(m_pGraphicsDevice->GetDevice()->CreateInputLayout(
				InputElementDescUV,
				sizeof(InputElementDescUV) / sizeof(InputElementDescUV[0]),
				m_pVertexCompiledShader->GetBufferPointer(),
				m_pVertexCompiledShader->GetBufferSize(),
				&m_pVertexLayout)))
			{
		...
		bool MultipleVertex2DUV::CreatePixelShader()
		{
			UINT Flag1 = D3D10_SHADER_ENABLE_STRICTNESS;
#ifdef _DEBUG
			Flag1 |= D3D10_SHADER_DEBUG | D3D10_SHADER_SKIP_OPTIMIZATION;
		...
		bool MultipleVertex2DUV::CreateState()
		{
		...
		void MultipleVertex2DUV::ReleaseVertexShader()
		{
			SafeRelease(m_pVertexShader);
			SafeRelease(m_pVertexCompiledShader);
		}

		void MultipleVertex2DUV::ReleaseVertexLayout()
		{
			SafeRelease(m_pVertexLayout);
		}

		void MultipleVertex2DUV::ReleasePixelShader()
		{
			SafeRelease(m_pPixelShader);
			SafeRelease(m_pPixelCompiledShader);
		}

		void MultipleVertex2DUV::ReleaseState()
		{
