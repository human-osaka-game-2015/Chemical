...
namespace Game
{
	...
	//----------------------------------------------------------------------
	WarpExitGimmick::WarpExitGimmick() :
		...
		m_pExitEvent(new WarpExitEvent(WARP_EXIT_EVENT)),
	...
	// Public Functions
	//----------------------------------------------------------------------
	bool WarpExitGimmick::Initialize()
	{
		...
		m_pReciveFunc = std::bind(&WarpExitGimmick::ReciveEvent, this, std::placeholders::_1);
		...
		while (!m_pCollision->IsCollisionDataEmpty())
		{
			...
			if (Data.OtherId == PLAYER_COLLISION_ID)
			{
				...
				for (unsigned int i = 0; i < pRectangles->size(); i++)
				{
					...
					{
						m_pExitEvent->SetWarpId(m_GimmickData[i]);
	...
	void WarpExitGimmick::AddGimmick(int _x, int _y, int _data)
	{
	...
	void WarpExitGimmick::ClearChip()
	{
	...
	bool WarpExitGimmick::CreateInstanceBuffer()
	{
		// ギミックの数が0なら生成しない.
	...
	void WarpExitGimmick::ReleaseInstanceBuffer()
	{
	...
	// Private Functions
	//----------------------------------------------------------------------
	void WarpExitGimmick::ReciveEvent(Lib::EventBase* _pEvent)
	{
		if (_pEvent->GetEventID() == WARP_ENTRANCE_EVENT)
		{
			WarpEntranceEvent* pEvent = reinterpret_cast<WarpEntranceEvent*>(_pEvent);
