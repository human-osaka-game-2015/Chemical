...
namespace Game
{
	...
	//----------------------------------------------------------------------
	BlueButtonGimmick::BlueButtonGimmick() :
		StageGimmickBase("Resource\\GameScene\\Texture\\Gimmick13.png", "BlueButtonGimmick"),
		m_pCollision(new ButtonGimmickCollision(BUTTON_GIMMICK_COLLISION_ID)),
		m_pEvent(new BlueButtonGimmickEvent(BLUE_BUTTON_GIMMICK_EVENT))
	{
	}
	...
	// Public Functions
	//----------------------------------------------------------------------
	bool BlueButtonGimmick::Initialize()
	{
		...
		m_pReciveFunc = std::bind(&BlueButtonGimmick::ReciveEvent, this, std::placeholders::_1);
	...
	void BlueButtonGimmick::Finalize()
	{
	...
	void BlueButtonGimmick::AddGimmick(int _x, int _y, int _data)
	{
		...
		m_GimmickUV.emplace_back(D3DXVECTOR2(0.5f, 0.0f));
		m_GimmickData.emplace_back(_data);
		m_pCollision->AddRect(Rect);
		m_GimmickNum++;
	}

	void BlueButtonGimmick::ClearChip()
	{
	...
	bool BlueButtonGimmick::CreateInstanceBuffer()
	{
		// ギミックの数が0なら生成しない.
	...
	void BlueButtonGimmick::ReleaseInstanceBuffer()
	{
	...
	// Private Functions
	//----------------------------------------------------------------------
	void BlueButtonGimmick::ReciveEvent(Lib::EventBase* _pEvent)
	{
		if (_pEvent->GetEventID() == BLUE_BLOCK_GIMMICK_EVENT)
		{
		} 
