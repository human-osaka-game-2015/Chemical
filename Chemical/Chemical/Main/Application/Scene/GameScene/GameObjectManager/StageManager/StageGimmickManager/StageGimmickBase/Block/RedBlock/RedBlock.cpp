...
namespace Game
{
	...
	//----------------------------------------------------------------------
	RedBlock::RedBlock() :
		StageGimmickBase("Resource\\GameScene\\Texture\\Gimmick16.png", "RedBlock"),
	...
	void RedBlock::AddGimmick(int _x, int _y, int _data)
	{
	...
	bool RedBlock::CreateInstanceBuffer()
	{
		// ギミックの数が0なら生成しない.
	...
	void RedBlock::ReleaseInstanceBuffer()
	{
	...
	// Private Functions
	//----------------------------------------------------------------------
	void RedBlock::ReciveEvent(Lib::EventBase* _pEvent)
	{
		if (_pEvent->GetEventID() == RED_BUTTON_GIMMICK_EVENT)
		{
